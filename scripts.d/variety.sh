...
#Install workload
if [[ $(find /etc/apt/ -name "*.list" | xargs cat | grep -c "variety") -eq 0 ]]
then
        run_privileged "Adding variety repository" "apt-add-repository" "-y" "ppa:variety/stable"
        run_privileged "Running apt update" "apt-get" "update"
        run_privileged "Installing variety" "apt-get" "install" "-y" "variety"
else
        echo "INFO: variety repository already setup"
fi
